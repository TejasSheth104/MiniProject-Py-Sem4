...
def limit_keypress_user(keypress_count,box_num):

# if the respective value of dictionary is 0, then its changed to 1 & hence cant be used again.
    if keypress_count_user[box_num]==0:
        keypress_count_user[box_num]=1
        return True
...
# sets the value (X/O) at respective position.        
        if win_con.win_cond(terminate_user,player1,player2,counter_user,keypress_count_user):
...
#     if chance.who_plays(counter_comp)=='X':
# # if value of dictionary is 1 already(returning Value FALSE.), no change takes place.
...
#     elif chance.who_plays(counter_comp)=='O':
